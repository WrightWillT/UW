...
**Summary:** We started by confirming the relative nomality of the residual distributions. Without knowing the variance of the temp = 120 condition, we tested several possibilities.  Ultimately, we recommend the most conservative option of n = 92 via a Welch's t-test simulation with unequal variance where we assume the variance of the temp = 120 condition is equal to the higher variance of the temp = 50 condition.  The justification for this choice is that the Welch t-test allows for different variances and sample sizes between the two samples. We assumed a variety of variances for testing, but the real variance is still an unknown. Without having this information, we took a conservative estimate with our recommendation. The Welch model also provides some flexibility in the event there are sampling errors and we need to discard any data points.
