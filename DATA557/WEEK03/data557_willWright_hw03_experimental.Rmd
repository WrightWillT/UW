...
To examine the relationship between sample sizes, power, and equal/unequal variances we investigated 5 conditions.  When assuming equal variance, we use the variance from the temp = 100 condition.  When assuming unequal variance, we tested two variants: using the modeled variance from the linear model above as well as the variance from the temp = 50 condition.  The reasoning with the latter is that it's possible that the temp = 100 condition is a minimum point of variance and that it climbs rapidly as the temperature increases.
...
3. Assuming a t-distribution with the modeled variance for temp = 120 using a T-statistic (Welch's T-test)

4. Assuming a t-distribution with unequal variance where temp = 120 has the variance of the temp = 50 condition, using a T-statistic (Welch's T-test)

5. Assuming a t-distribution with equal variance to temp = 100 using a T-statistic (Welch's T-test)
...
powers_w_unequalVar <- rep(NA,length(n_vals))
...
for(j in 1:length(n_vals)){
  ...
  welch_df_unequalVar <- rep(NA,reps)
  for(i in 1:reps){
    ...
    temp_120sim_sd_unequalVar <- sd(temp_120sim_unequalVar)
    ...
    welch_df_unequalVar[i] <- (temp_100sim_sd^2/n_vals[j] + temp_120sim_sd_unequalVar^2/n_vals[j])^2/
      (temp_100sim_sd^4/(n_vals[j]^2*(n_vals[j]-1)) + temp_120sim_sd_unequalVar^4/(n_vals[j]^2*(n_vals[j]-1))) 
  }
  ...
  welch_df_unequalVar <- mean(welch_df_unequalVar)
  powers_w_equalVar[j] <- mean(abs(test_statistic_equalVar)>qt(0.975, df = welch_df_equalVar)) # welch's t-test
  powers_w_unequalVar[j] <- mean(abs(test_statistic_unequalVar)>qt(0.975, df = welch_df_unequalVar)) # welch's t-test
}
results <- data.frame(n = rep(n_vals,4), 
                      test_type = c(rep("Z-test", length(n_vals)),
                                     rep("T-test", length(n_vals)),
                                     rep("Welch's T-test, Equal Var", length(n_vals)),
                                     rep("Welch's T-test, Modeled Var", length(n_vals))),
                      power = c(powers_z, powers_t, powers_w_equalVar, powers_w_unequalVar))
...
ideal_n_w_unequalVar <- min(results$n[which(results$power>=0.9 & results$test_type == "Welch's T-test, Modeled Var")])
...
g + geom_line() + 
  ...
  geom_vline(xintercept = ideal_n_w_unequalVar, col = "#00BFC4", linetype = 3) +
...
The results of this power analysis show that the needed sample size is `r ideal_n_z`, `r ideal_n_t`, `r ideal_n_w_equalVar`, or `r ideal_n_w_unequalVar` depending on if we wanted to go with a Z-test, T-test, Welch's T-test with equal variance, or Welch's T-test with modeled variance, respectively. 

Recommendation:
All the tests agree that a sample of 75 achieves at least a power of 90%. Because the modeled variance is smaller for the temp = 120 condition, Welch's T-test using this variance produced a smaller ideal n of 73. While it is likely the case that the smaller variance, we'd prefer to take a more conservative approach and assume the variance is equal to the higher standard deviation at temp = 100. This being the case, we recommend a sample size of 75. 
